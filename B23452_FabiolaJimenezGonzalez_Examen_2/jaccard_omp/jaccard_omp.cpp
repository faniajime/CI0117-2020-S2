...
int main(int argc, char *argv[])
{
    ...
    while (getline(matrix_file, matrix_row) && getline(text_file, text_row)) {
        ...
        int matrix_length = matrix_row.length();
        
        #pragma omp parallel for default(none) shared(matrix_length, test_vector, matrix_row) reduction(+:words_union, words_intersection)
